...
subs <- data.frame(
  ...
  "subs_kPa" = c(30, 30, 0.5, 0.5, 0.5, 0.5, 500),
  "subs_type_I" = as.factor(c(
    "Collagen", "Fibronectin",
    "Collagen", "Fibronectin",
    "Hyaluronic acid", "Hyaluronic acid",
    "Glass"
  )),
  "subs_type_II" = as.factor(c(
    "Collagen", "Fibronectin",
    "Collagen", "Fibronectin",
    "Collagen", "Fibronectin",
    "Glass"
...
plot_feature <- function(data, f, margins, offset) {
  ...
  if (f == "area") {
    ylabel <- bquote(.(yLab) ~ "Area (" * mu * "m"^2 * ")")
  } else if (f == "circularity") {
    ylabel <- paste(yLab, "Circularity")
  } else if (f == "aspect_ratio") {
    ylabel <- paste(yLab, "Aspect Ratio")
  } else if (f == "cell_stiffness") {
    ylabel <- paste(yLab, "Cell Stiffness (Pa)")
  } else if (f == "motility") {
  ...
  points(temp$subs_kPa, temp$median,
    type = "b", col = "blue", pch = 19, cex = 2.3, lwd = 2.5,
  ...
  points(temp$subs_kPa, temp$median,
    type = "b", col = "red", pch = 1, cex = 2.3, lwd = 2.5,
    xlim = x.axis.limits, ylim = y.axis.limits
  )
  ...
  points(temp$subs_kPa, temp$median,
    col = "red", pch = 2, cex = 2.3, lwd = 2.5,
    xlim = x.axis.limits, ylim = y.axis.limits
  )

  # re-plot Glass to avoid plot gridlines passing on top of the glass points
  temp <- data[[f]] %>% filter(grepl("Glass", sub_id))
  points(temp$subs_kPa, temp$median,
    col = "black", pch = 15, cex = 2.3,
    xlim = x.axis.limits, ylim = y.axis.limits
  ...
  text(
    y = par()$usr[3] - offset, x = 1200, "Glass",
...
png(paste0(cell_line, ".png"),
  res = 300, width = 2500, height = 2500, units = "px"
)
...
lgd <- temp %>%
  ...
  scale_shape_manual(values = c(
    "Collagen" = 19, "Fibronectin" = 1,
    "HA Collagen" = 17, "HA Fibronectin" = 2, "Glass" = 15
  )) +
  scale_color_manual(values = c(
    "Collagen" = "blue", "Fibronectin" = "red",
  ...
  theme(
    ...
    # order horizontally into 2 rows
    legend.position = "bottom",
    legend.box = "horizontal"
  ) +
  # arrange legend into 2 rows
  guides(
