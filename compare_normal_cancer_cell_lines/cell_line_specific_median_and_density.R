...
decimal <- list(
  "area" = 0, "circularity" = 2, "aspect_ratio" = 2,
...
for (f in features) {
  ...
  data.list <- setNames(
    vector("list", length(cell.line.label.colors.temp$cl_id)),
  ...
  # save the heatmap
  if (f %in% c("area", "aspect_ratio", "circularity")) {
    png(paste("../Figures/Supplementary_Figure6/", f, ".png", sep = ""),
      res = 300, width = 1500, height = 3000
    )
  } else {
    png(paste("../Figures/Supplementary_Figure7/", f, ".png", sep = ""),
      res = 300, width = 1500, height = 3000
    )
  }
  ...
  decorate_annotation("dens", {
    if (f != "motility") {
      ...
      grid.text(xlab[[f]],
        x = unit(0.5, "npc"),
        y = unit(-1.85 * nrow(temp.matrix), "line"),
        gp = gpar(fontsize = 12)
      )
      grid.text("Median\nvalue",
        x = unit(-0.27, "npc"),
  ...
  col_fun <- colorRamp2(
    seq(min(temp.matrix), max(temp.matrix), length.out = 12),
    hcl.colors(12, "Blue-Red")
  )
  if (f %in% c("area", "aspect_ratio", "circularity")) {
    png(paste("../Figures/Supplementary_Figure6/", f, "_legend.png", sep = ""),
      res = 300, width = 1000, height = 200
    )
  } else {
    png(paste("../Figures/Supplementary_Figure7/", f, "_legend.png", sep = ""),
      res = 300, width = 1000, height = 200
    )
  }
