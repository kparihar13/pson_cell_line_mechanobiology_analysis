...
substrate.color <- c(
  ...
  "#D55E00", "#EEDC5B", "#56B4E9", "#CC79A7"
)
names(substrate.color) <- c(
  "500 Pa Coll", "500 Pa FN", "30 kPa Coll",
  "30 kPa FN", "HA FN", "HA Coll", "Glass"
)
...
data <- lapply(data, function(x) {
  x$sub_id <- gsub("500Pa", "500 Pa", x$sub_id)
  x$sub_id <- gsub("30kPa", "30 kPa", x$sub_id)
  return(x)
})
...
ggsave("../Figures/Supplementary_Figure10/pca_variance_loadings.png",
...
pca.res.df <- pca.res$x[, 1:4] %>%
  ...
  mutate(
    cl_id = mapply(function(x) strsplit(x, "_")[[1]][1], cl_sub),
...
ggsave("../Figures/Supplementary_Figure10/pca_tissue.png",
  tissue.pca.plots,
...
ggsave("../Figures/Supplementary_Figure10/pca_substrate.png",
  subs.pca.plots,
  dpi = 300, units = "px", width = 2500, height = 3500
)
